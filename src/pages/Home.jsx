...
const FEATURES = [
  { icon: Zap,    title: 'Reserva Instantánea', desc: 'Confirmación inmediata sin llamadas' },
  { icon: Clock,  title: 'Horarios Flexibles',  desc: 'Abierto todos los días de 8 a 23 hs' },
  { icon: Shield, title: 'Cancelás sin cargo',  desc: 'Hasta 2 horas antes del turno' },
  { icon: Star,   title: 'Calidad Premium',     desc: 'Césped sintético certificado FIFA' },
]
    ...
    <>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-24">
          <p className="section-label mb-4 animate-fade-up opacity-0" style={{ animationFillMode: 'forwards' }}>
          ...
          >
            Reservá tu cancha de fútbol 5 o fútbol 7 en segundos. Sin llamadas, sin complicaciones.
          </p>
          <div
            className="flex flex-col sm:flex-row gap-3 animate-fade-up opacity-0"
            style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}
          >
            <Link to="/reservas" className="btn-primary flex items-center justify-center gap-2">
          ...
          >
            {[
              ...
              <div key={label}>
                <div className="font-display font-black text-verde-400 text-3xl">{num}</div>
                <div className="font-mono text-carbon-400 text-xs uppercase tracking-widest">{label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex flex-col gap-3 group">
                ...
                <div>
                  <h3 className="font-display font-bold text-white text-sm uppercase tracking-wide">{title}</h3>
      ...
      <section className="py-20 max-w-6xl mx-auto px-4">
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          <div>
            ...
            <h2 className="section-title text-4xl md:text-5xl">Nuestras canchas</h2>
            <p className="text-carbon-300 mt-3 text-sm max-w-xl">Elegí tu cancha y reservá el horario que más te convenga.</p>
          </div>
          <Link to="/reservas" className="btn-ghost flex items-center gap-1 mb-10">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="font-mono text-carbon-400 text-xs uppercase tracking-widest">Cargando canchas...</p>
          </div>
        ) : canchas.length === 0 ? (
          <p className="text-carbon-400 text-sm">No hay canchas disponibles todavía.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {canchas.map((c) => <CanchaCard key={c._id} cancha={c} />)}
          </div>
        )}
      </section>
      ...
      <section className="bg-verde-600 py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="font-mono text-verde-900/70 text-xs uppercase tracking-widest mb-3">¿Listo para jugar?</p>
          <h2 className="font-display font-black text-carbon-900 text-5xl md:text-6xl uppercase leading-none mb-6">
            Reservá tu turno<br />en menos de 2 minutos
          </h2>
          <Link
            to="/reservas"
        ...
        <section className="py-20 max-w-6xl mx-auto px-4">
          <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
            <div>
              ...
              <h2 className="section-title text-4xl md:text-5xl">Tienda deportiva</h2>
              <p className="text-carbon-300 mt-3 text-sm max-w-xl">Todo lo que necesitás para jugar al máximo.</p>
            </div>
            <Link to="/productos" className="btn-ghost flex items-center gap-1 mb-10">
